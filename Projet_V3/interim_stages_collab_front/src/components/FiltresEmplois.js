...
export function FiltresEmplois({ data, onConfirm, onDismiss }) {
	...
	const handleSubmit = async () => {
		const filteredData = data.filter((item) => {
			return (
				(!startDate ||
					moment(item.offre.debut).isSameOrBefore(moment(endDate), "day")) &&
				(!endDate ||
					moment(item.offre.fin).isSameOrAfter(moment(startDate), "day")) &&
